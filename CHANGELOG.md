...
### Fixed
- **Sync no longer depends on controller-local deployment intent.** It discovers live models
  from the hosts registered on the machine running `omw`, so deploying or stopping from another
  controller cannot hide endpoints or leave stale discovery state behind.
...
- **loom v3: deterministic packet composition.** Role instructions and the Return
  ...
  (regenerated by sync), repo overlays in ./.loom/skills/ (override XOR default+local),
  contract at the packet tail. Workers get explicit per-dispatch models; a compose
  ledger event records exactly what each worker received.
- **Team orchestrator and agent-instruct removed.** Contract drops the
  "NEXT STEPS FOR team" field; sync prunes all stale deployed artifacts.
- **Nudge merges instead of discarding**; plan handoff carries the full deliverable
  (strip_contract_tail), not the RESULT summary. agent-runbook-review replaced by
  session-review. Ask guard: an ask must be the first loom call since the user last
  spoke (kills orchestrator self-research loops). Stale-server tripwire diffs the
  attach target agent registry against on-disk config. cmd_pr routes PR creation to
  agent-test; approval/merge stays with agent-review.
