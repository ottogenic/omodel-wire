...
//
// Tokens are read from (chmod 600):
//   ~/.config/otools/gh_token_coder      (shared bot account)
//   ~/.config/otools/gh_token_reviewer   (your account)
import { readFileSync } from "fs";
import { homedir } from "os";
import { join } from "path";

const tok = (name) => {
  try { return readFileSync(join(homedir(), ".config", "otools", name), "utf8").trim(); }
  catch { return ""; }
};
